...
public class BankTester {
    ...
    static class FlatPercentProvisionTransaction extends Transaction {
        ...
        @Override
        public String getProvision() {
            return (int) (Bank.getAmountFloat(getAmount())) * getPercent() / 100. + "$";
        }
    }
    ...
    static class Bank {
        ...
        public Optional<Account> chechAccount(long id) {
        ...
        public boolean checkBalance(Account account, String amount, String provision) {
            return getAmountFloat(account.getBalance()) > (getAmountFloat(amount) + getAmountFloat(provision));
        }

        private void changeSum(String amount, String provision) {
            this.transfer = String.format(Locale.US, "%.2f$", getAmountFloat(transfer) + getAmountFloat(amount));
            this.provision = String.format(Locale.US, "%.2f$", getAmountFloat(this.provision) + getAmountFloat(provision));
        }

        private void accountChanges(Account send, Account receive, String amount, String provision) {
            send.setBalance(String.format(Locale.US, "%.2f$", getAmountFloat(send.getBalance()) - getAmountFloat(amount) - getAmountFloat(provision)));
            receive.setBalance(String.format(Locale.US, "%.2f$", getAmountFloat(receive.getBalance()) + getAmountFloat(amount)));
        }

        public boolean makeTransaction(Transaction t) {
            Optional<Account> sender = chechAccount(t.fromId);
            Optional<Account> receiver = chechAccount(t.toId);
            ...
            if (sender.isPresent() && receiver.isPresent()) {
                ...
                if (checkBalance(senderAccount, t.getAmount(), provision)) {
                    accountChanges(senderAccount, receiverAccount, t.amount, provision);
        ...
        public boolean equals(Object o) {
            ...
            if (o == null || getClass() != o.getClass()) return false;
            Bank bank = (Bank) o;
            return name.equals(bank.name) && Arrays.equals(accounts, bank.accounts) && transfer.equals(bank.transfer) && provision.equals(bank.provision);
        }
